...
public class Session {
	...
	public Protocol authenticate(String username, String password){
		...
		buffer.appendBytes(this.authHash);
		buffer.appendBytes(this.clientRandom);
		buffer.appendBytes(this.serverRandom);
		buffer.appendByte((byte)0x00); /* Changed later */
	...
	private void generateAuthHash(){
		...
		buffer.appendBytes(this.salt);
		buffer.appendString(" ");
		buffer.appendBytes(this.password);
		
		this.authHash = Hash.sha1(buffer.getBytes());
	}
	...
	private void generateAuthHmac(){
		...
		buffer.appendBytes(this.clientRandom);
		buffer.appendBytes(this.serverRandom);
		buffer.appendBytes(this.dhClientKeyPair.getPublicKeyBytes());
		buffer.appendBytes(DH.keyToBytes(this.dhServerPublicKey));
		buffer.appendBytes(this.rsaClientKeyPair.getPublicKeyBytes());
		buffer.appendByte((byte)this.username.length);
		buffer.appendBytes(this.username);
		buffer.appendByte((byte)0x01);
		buffer.appendByte((byte)0x40);
		
		this.authHmac = Hash.hmacSha1(buffer.getBytes(), this.keyHmac);
	}
	...
	private void solvePuzzle(){
		...
		do {
			...
			buffer.clear();
			buffer.appendBytes(this.serverRandom);
			buffer.appendBytes(this.puzzleSolution);
			
			/* Calculate digest. */
			digest = Hash.sha1(buffer.getBytes());
